...
class Network extends haxor.net.client.Network
{
	...
	override private function OnData(p_data : String):Void
	{
		...
		switch(code)
		{
			...
			case Protocol.STC_UPDATEPLAYER :	game.updatePlayer(data.id, data.x, data.y, data.rotation);
	...
	public function updatePosition(x : Float, y : Float, rotation : Float)
	{
		Send({code : Protocol.CTS_UPDATEPLAYER, x : x, y : y, rotation : rotation});
	}
